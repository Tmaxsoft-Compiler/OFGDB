...
/* set additional attributes of PL/I variables */
static struct type* set_pli_attr (struct type* type, const char* name) {
    int buf_len = strlen (name), count = 1;
    char* type_name = NULL;
    char* buf = (char*) alloca (buf_len);
    memcpy (buf, name, buf_len);

    type_name = strtok (buf, "!");
    if (type_name != NULL) {
        /* FIXED DEC, FIXED BIN type: get digit, scale, endianity */
        if ( strstr (type_name, "FIXED") != NULL) {
            char* digit_char = strtok(NULL, "!");
            char* scale_char = strtok(NULL, "!");
            char* endian_char = strtok(NULL, "!");

            gdb_assert(digit_char != NULL && "Invalid type info of FIXED type");
            gdb_assert(scale_char != NULL && "Invalid type info of FIXED type");
            gdb_assert(endian_char != NULL && "Invalid type info of FIXED type");

            TYPE_PLI_DIGIT(type) = atoi(digit_char);
            TYPE_PLI_SCALE(type) = atoi(scale_char);
            TYPE_PLI_ENDIAN(type) = atoi(endian_char);
        }
        /* FLOAT DEC, FLOAT BIN: get digit */
        else if ( strstr (type_name, "FLOAT") != NULL) {
            char* digit_char = strtok(NULL, "!");

            gdb_assert(digit_char != NULL && "Invalid type info of FIXED type");

            TYPE_PLI_DIGIT(type) = atoi(digit_char);
            TYPE_PLI_SCALE(type) = 0;
        }
        /* BIT: get flag for length, varying, endianity */
        else if ( strcmp (type_name, "BIT") == 0) {
            char* real_bit_size = strtok(NULL, "!");
            char* bit_offset = strtok(NULL, "!");
/*            char* aligned_flag = strtok(NULL, "!"); */
            char* varying_flag = strtok(NULL, "!");
            char* endian_char = strtok(NULL, "!");

            gdb_assert(real_bit_size != NULL && "Invalid type info of BIT type");
            gdb_assert(bit_offset != NULL && "Invalid type info of BIT type");
/*            gdb_assert(aligned_flag != NULL && "Invalid type info of BIT type"); */
            gdb_assert(varying_flag != NULL && "Invalid type info of BIT type");
            gdb_assert(endian_char != NULL && "Invalid type info of BIT type");

            TYPE_PLI_BITSIZE(type) = atoi(real_bit_size);
            TYPE_PLI_BITOFFSET(type) = atoi(bit_offset);
/*            TYPE_PLI_ALIGNED(type) = atoi(aligned_flag); */
            TYPE_PLI_VARYING(type) = atoi(varying_flag);
            TYPE_PLI_ENDIAN(type) = atoi(endian_char);
        }
        /* CHARACTER,GRAPHIC, WIDECHAR: get flag for varying, endianity */
        else if ( strcmp (type_name, "CHARACTER") == 0 ||
                  strcmp (type_name, "GRAPHIC") == 0 ||
                  strcmp (type_name, "WIDECHAR") == 0 ) {
            char* length_char = strtok(NULL, "!");
            char* bit_offset = strtok(NULL, "!");
            char* varying_flag = strtok(NULL, "!");
            char* endian_char = strtok(NULL, "!");

            gdb_assert(length_char != NULL && "Invalid type info of BIT type");
            gdb_assert(varying_flag != NULL && "Invalid type info of CHAR/GRAPHIC/WIDECHAR type");
            gdb_assert(endian_char != NULL && "Invalid type info of CHAR/GRAPHIC/WIDECHAR type");

            TYPE_PLI_LENGTH(type) = atoi(length_char);
            TYPE_PLI_BITOFFSET(type) = atoi(bit_offset);
            TYPE_PLI_VARYING(type) = atoi(varying_flag);
            TYPE_PLI_ENDIAN(type) = atoi(endian_char);
        }
        /* PICTURE: get picture string*/
        else if ( strcmp (type_name, "PICTURE") == 0) {
            char* pic_str_buf = strtok (NULL, "!");
            char* pic_str = TYPE_PLI_PIC_STR (type);
            int size_pic_str_buf = strlen (pic_str_buf);

            gdb_assert(pic_str_buf != NULL && "Invalid type info of PICTURE type");
           
            memcpy (pic_str, pic_str_buf, size_pic_str_buf);
            pic_str[size_pic_str_buf] = '\0';
        }
    }
    ...
    switch (encoding)
    {
        ...
        case DW_ATE_signed:
            if (cu->language == language_pli 
                && name
                && strncmp (name, "FIXED BIN", sizeof ("FIXED BIN") - 1) == 0) 
                code = TYPE_CODE_SIGNED_FIXED_B;
            break;
        case DW_ATE_unsigned:
            ...
            if (cu->language == language_fortran
                ...
                code = TYPE_CODE_CHAR;
                     ...
                     && name
                     && strncmp (name, "FIXED BIN", sizeof ("FIXED BIN") - 1) == 0) 
                code = TYPE_CODE_UNSIGNED_FIXED_B;
            break;
        ...
        case DW_ATE_unsigned_char:
            if (cu->language == language_ada || cu->language == language_m2
                ...
                if (name) {
                    if (strncmp (name, "BIT", sizeof ("BIT") - 1) == 0)
                        code = TYPE_CODE_BIT;
                    else if (strncmp (name, "CHARACTER", sizeof ("CHARACTER") - 1) == 0)
                        code = TYPE_CODE_CHAR;
                    else if (strncmp (name, "GRAPHIC", sizeof ("GRAPHIC") - 1) == 0)
                        code = TYPE_CODE_DBCS;
                }
            }
            type_flags |= TYPE_FLAG_UNSIGNED;
