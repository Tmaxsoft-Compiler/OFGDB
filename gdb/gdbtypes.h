...
/* precision of FIXED DEC */
struct pli_precision_attr {
    int16_t digit;
    int16_t scale;
};

/* for CHAR, BIT, WIDECHAR, GRAPHIC type _ 1: VARYING char / 0: NONVARYING */
struct pli_str_attr {
    int8_t is_varying; 
    int16_t size;
    int16_t offset;
...
/* additional info. of each type */
union pli_additional_attr {
    struct pli_str_attr str_attr; /* for STRING type */
    struct pli_precision_attr prec; /* for FIXED DEC, FIXED BIN type */
    char pic_string[512]; /* for PICTURE type */
};

/* additional info. of PL/I variables */
struct pli_attribute {
...
#define TYPE_PLI_DIGIT(thistype) (thistype)->pli_attr->additional_attr.prec.digit
#define TYPE_PLI_SCALE(thistype) (thistype)->pli_attr->additional_attr.prec.scale
#define TYPE_PLI_LENGTH(thistype) (thistype)->pli_attr->additional_attr.str_attr.size
#define TYPE_PLI_BITSIZE(thistype) (thistype)->pli_attr->additional_attr.str_attr.size
#define TYPE_PLI_BITOFFSET(thistype) (thistype)->pli_attr->additional_attr.str_attr.offset
#define TYPE_PLI_VARYING(thistype) (thistype)->pli_attr->additional_attr.str_attr.is_varying
